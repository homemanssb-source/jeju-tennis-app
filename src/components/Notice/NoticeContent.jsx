...
// 공통 날짜 포맷
export function formatNoticeDate(s) {
  if (!s) return ''
  const d = new Date(s)
...
// 대회공지 카드 (목록용)
export function TournamentCard({ n, onSelect }) {
  const m = n.meta || {}
  return (
    <button onClick={() => onSelect(n)}
      ...
      <div className="px-4 py-3">
        ...
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs text-gray-400">{formatNoticeDate(n.created_at)}</span>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className="text-[11px] text-sub">{formatNoticeDate(n.created_at)}</span>
            {n.link && (
...
// 공지 목록 (대회/일반 카드 자동 분기)
export function NoticeList({ notices, onSelectTournament }) {
  return (
    <div className="space-y-2">
      {notices.map(n => (
        n.notice_type === 'tournament'
          ? <TournamentCard key={n.id} n={n} onSelect={onSelectTournament} />
          : <GeneralCard key={n.id} n={n} />
      ))}
    </div>
  )
}
...
// 부서별 참가자격 이미지 토글
export function QualificationImageSection({ imageUrl }) {
...
// 대회 상세 (선택된 notice의 전체 디테일)
export function TournamentDetailView({ notice }) {
  const m = notice.meta || {}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-400 rounded-2xl p-5 text-white">
        ...
        <h1 className="text-lg font-black leading-tight">{notice.title}</h1>
      ...
      <p className="text-xs text-sub text-center">{formatNoticeDate(notice.created_at)} 등록</p>
    </div>
  )
}
